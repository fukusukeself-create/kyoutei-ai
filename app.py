...
if "GEMINI_API_KEY" in st.secrets:
    ...
    GEMINI_API_KEY = st.sidebar.text_input("Gemini API Key を入力（Secrets未設定時）", type="password")

# ----------------------------------------------------
# 2. フォーメーション正確計算エンジン（重複排除・点数計算）
# ----------------------------------------------------
def parse_and_expand_formation(formation_str):
    """
    フォーメーション文字列（例: '1-23-2345' や '12-123-2345'）から
    昇順整列・重複排除を行い、正確な点数と買い目リストを算出する
    """
    try:
        parts = formation_str.strip().replace(" ", "").split("-")
        if len(parts) != 3:
            return formation_str, [], 0

        # 各桁の数字を取り出してソート・重複排除（昇順）
        ...
        clean_second = "".join(map(str, second_second := second_ranks))
...
STADIUM_MASTER = [
    ...
    {"id": "08", "name": "常滑", "type": "day", "status": "一般 3日目", "status_sub": "発売終了", "r": "12R", "time": "終了"},
    ...
    {"id": "13", "name": "尼崎", "type": "day", "status": "一般 初日", "status_sub": "発売終了", "r": "12R", "time": "終了"},
    {"id": "14", "name": "鳴門", "type": "day", "status": "開催なし", "r": "-", "time": "--:--"},
    {"id": "15", "name": "丸亀", "type": "night", "status": "開催なし", "r": "-", "time": "--:--"},
    {"id": "16", "name": "児島", "type": "day", "status": "一般 4日目", "status_sub": "発売終了", "r": "12R", "time": "終了"},
    {"id": "17", "name": "宮島", "type": "day", "status": "開催なし", "r": "-", "time": "--:--"},
    {"id": "18", "name": "徳山", "type": "day", "status": "一般 2日目", "status_sub": "発売終了", "r": "12R", "time": "終了"},
    {"id": "19", "name": "下関", "type": "night", "status": "一般 4日目", "r": "7R", "time": "18:15", "active": True},
    {"id": "20", "name": "若松", "type": "night", "grade": "G3", "day_num": "2日目", "status": "8R", "time": "18:40", "active": True},
    {"id": "21", "name": "芦屋", "type": "day", "status": "一般 3日目", "status_sub": "発売終了", "r": "12R", "time": "終了"},
...
・本命（的中重視・4〜8点目安）、抑え（保険・2〜6点目安）、穴（高配当・6〜12点目安）のフォーメーション文字列を出力すること。
...
<div class="main-header">
    ...
    <div style="font-size:12px;">高精度フォーメーション自動計算</div>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    ...
    for idx, item in enumerate(STADIUM_MASTER):
        with cols[idx % 4]:
            ...
            grade_badge = f"<span class='card-grade'>{item.get('grade')}</span> " if "grade" in item else ""
            ...
            <div style="background:{bg_color}; padding:10px; border-radius:8px; margin-bottom:8px; {active_border} text-align:center;">
                <div style="font-size:16px; font-weight:bold; color:#111;">{is_night}{item['name']}</div>
                <div style="font-size:11px; margin-top:2px;">{grade_badge}{item.get('status', '')}</div>
                <div style="font-size:13px; font-weight:bold; color:#d32f2f; margin-top:4px;">{item['r']} {item['time']}</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    for r in TIMELINE_RACES:
        ...
        with c4:
            if st.button("予想を見る", key=f"btn_{r['stadium']}_{r['round']}"):
                ...
                st.toast(f"{r['stadium']} {r['round']}R を読み込みました！")
        st.divider()
