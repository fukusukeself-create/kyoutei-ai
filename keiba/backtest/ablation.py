...
for start, end in T.FOLDS:
    ...
    career, form = {}, {"j": {}, "t": {}}
    tables = F.new_tables()
    df_tr = T.build_rows(races[races.date < start], tr_r, stats, career, form=form, tables=tables)
    df_va = T.build_rows(va_races, runners[runners.race_id.isin(va_races.race_id)], stats, career, form=form, tables=tables)
