...
def main():
    ...
    for spec, keep in SPECS:
        ...
        if ret < 0:
            print(f"JVOpen({spec}) に失敗しました (コード {ret})。-301〜-303 は利用キー・認証の問題、-504 はサーバーメンテナンス中。")
            continue
        ...
        files = {k: open(os.path.join(OUT, f"{spec}_{k}.txt"), "a", encoding="utf-8", newline="\n") for k in keep}
        n = {k: 0 for k in keep}
        t0 = time.time()
        while True:
            r = jv.JVRead("", 110000, "")
    ...
    zpath = os.path.join(HERE, "jvdata.zip")
